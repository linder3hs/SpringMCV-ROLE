...
public class EmployeeController {
	...
	@PostMapping("/admin/emp/editsaveRole")
	public ModelAndView editsaveRole(@ModelAttribute("employee") @Valid Employee emp, BindingResult result,
			ModelMap model) {
		...
		if (result.hasErrors()) {
			modelAndView = new ModelAndView("admin/emp/editroleform","employee", emp);
		} else {
			try {
				employeeService.updateWithrole(emp.getLogin(), emp.getPassword(), emp.getFirstname(), emp.getLastname(),
						emp.getSalary(), 12, emp.getRole());
				modelAndView = new ModelAndView("redirect:/admin/emp/list");
	...
	// DELETE ROLE //
	///////////////////	
	@PostMapping("/admin/emp/deleterole")
	public ModelAndView deleterole(@ModelAttribute("employee") @Valid Employee emp, BindingResult result, ModelMap model) {

		ModelAndView modelAndView = null;

		if (result.hasErrors()) {
			modelAndView = new ModelAndView("admin/emp/deleteroleform/" + emp.getEmployeeId(), "employee", emp);
		} else {
			try {
				employeeService.delete(emp.getLogin());
				modelAndView = new ModelAndView("redirect:/admin/emp/list");
...
try {
employeeService.createWithRole(emp.getLogin(), emp.getPassword(), emp.getLastname(),emp.getFirstname(),
 emp.getSalary(), 12, emp.getRole().getRoleId());
logger.info("new Employee login = " + emp.getLogin());
modelAndView = new ModelAndView("redirect:/admin/emp/list");
} catch (Exception e) {
logger.error(e.getMessage());
